-- 1. S'assurer que toutes les marques existent
INSERT INTO brands (slug, name, country, verified) VALUES
  ('medicube', 'Medicube', 'Corée', true),
  ('anua', 'Anua', 'Corée', true),
  ('beauty-of-joseon', 'Beauty of Joseon', 'Corée', true),
  ('skin1004', 'SKIN1004', 'Corée', true),
  ('haruharu-wonder', 'Haruharu Wonder', 'Corée', true),
  ('mixsoon', 'Mixsoon', 'Corée', true),
  ('torriden', 'Torriden', 'Corée', true),
  ('eos', 'EOS', 'USA', true),
  ('kojie-san', 'Kojie San', 'Philippines', true),
  ('laneige', 'Laneige', 'Corée', true)
ON CONFLICT (slug) DO UPDATE SET verified = true;

-- 2. Masquer tous les anciens produits
UPDATE products SET is_active = false;

-- 3. Insérer les 14 nouveaux arrivages
INSERT INTO products (
  slug, name, brand_id, category_id, description, ingredients, usage_tips, precautions,
...
('medicube-deep-vita-c-serum', 'Medicube Deep Vita C Serum', 
 (SELECT id FROM brands WHERE slug = 'medicube' LIMIT 1), 1, 
...
('anua-niacinamide-10-txa-4-serum', 'Anua Niacinamide 10% + TXA 4% Serum', 
 (SELECT id FROM brands WHERE slug = 'anua' LIMIT 1), 1, 
...
('beauty-of-joseon-relief-sun', 'Beauty of Joseon Sun Relief (Riz + Probiotiques)', 
 (SELECT id FROM brands WHERE slug = 'beauty-of-joseon' LIMIT 1), 1, 
...
('beauty-of-joseon-glow-deep-serum', 'Beauty of Joseon Glow Deep Serum', 
 (SELECT id FROM brands WHERE slug = 'beauty-of-joseon' LIMIT 1), 1, 
...
('skin1004-hyalu-cica-sun-serum', 'SKIN1004 Hyalu-Cica Water-Fit Sun Serum', 
 (SELECT id FROM brands WHERE slug = 'skin1004' LIMIT 1), 1, 
...
('skin1004-centella-ampoule-foam', 'SKIN1004 Centella Ampoule Foam', 
 (SELECT id FROM brands WHERE slug = 'skin1004' LIMIT 1), 1, 
...
('haruharu-airyfit-sunscreen', 'Haruharu Wonder Airyfit Sunscreen SPF50+', 
 (SELECT id FROM brands WHERE slug = 'haruharu-wonder' LIMIT 1), 1, 
...
('haruharu-hyaluronic-toner', 'Haruharu Wonder Black Rice Hyaluronic Toner', 
 (SELECT id FROM brands WHERE slug = 'haruharu-wonder' LIMIT 1), 1, 
...
('mixsoon-bean-essence', 'Mixsoon Bean Essence', 
 (SELECT id FROM brands WHERE slug = 'mixsoon' LIMIT 1), 1, 
...
('torriden-dive-in-cream', 'Torriden DIVE-IN Low Molecular Hyaluronic Acid Cream', 
 (SELECT id FROM brands WHERE slug = 'torriden' LIMIT 1), 1, 
...
('laneige-lip-sleeping-mask-mini', 'Laneige Lip Sleeping Mask Berry', 
 (SELECT id FROM brands WHERE slug = 'laneige' LIMIT 1), 1, 
...
('eos-body-wash-coconut', 'EOS Body Wash Coconut Waters', 
 (SELECT id FROM brands WHERE slug = 'eos' LIMIT 1), 2, 
...
('eos-body-wash-vanilla', 'EOS Body Wash Vanilla Cashmere', 
 (SELECT id FROM brands WHERE slug = 'eos' LIMIT 1), 2, 
...
('kojie-san-soap-pack-3', 'Savon Kojie San Éclaircissant (Pack 3-en-1)', 
 (SELECT id FROM brands WHERE slug = 'kojie-san' LIMIT 1), 2, 
